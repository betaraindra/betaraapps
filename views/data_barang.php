...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['save_product'])) {
        try {
            ...
            $stock = (int)$_POST['stock']; // Biasanya stok diatur lewat opname/transaksi, tapi untuk inisialisasi boleh edit.
            ...
            if (!empty($_POST['edit_id'])) {
                ...
                // Logic Gambar: Jika upload baru, pakai baru. Jika tidak, pakai lama.
                $sql_img = "";
...
// --- FILTER & QUERY DATA ---
$start_date = $_GET['start'] ?? date('Y-m-01', strtotime('-1 year')); // Default range luas untuk master
$end_date = $_GET['end'] ?? date('Y-m-d');
$search = $_GET['q'] ?? '';
$cat_filter = $_GET['category'] ?? 'ALL';
$stock_status = $_GET['stock_status'] ?? 'ALL';
$sort_by = $_GET['sort'] ?? 'newest';
$warehouse_filter = $_GET['warehouse_id'] ?? 'ALL'; // Filter gudang (lokasi terakhir)
