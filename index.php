...
$standalone_pages = ['cetak_surat_jalan', 'cetak_barcode', 'download_backup', 'cetak_data_barang', 'cetak_laporan_gudang', 'export_excel_gudang'];
...
$menu_data_pages = ['data_barang', 'data_keuangan', 'data_transaksi', 'data_wilayah', 'detail_gudang'];
...
<body class="bg-gray-100 flex h-screen overflow-hidden font-sans">
        ...
        <div class="flex-1 overflow-y-auto sidebar-scroll py-4">
            <nav class="px-2 space-y-1">
                ...
                <a href="?page=input_aktivitas" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md hover:bg-slate-700 <?= $page == 'input_aktivitas' ? 'bg-slate-900 text-white' : 'text-slate-300' ?>">
                    <i class="fas fa-tools mr-3 w-6 text-center"></i> Input Aktivitas
                </a>
                <?php endif; ?>
                ...
                <div id="menu-laporan" class="<?= $is_menu_laporan_open ? '' : 'hidden' ?> space-y-1 mt-1 pl-2 border-l-2 border-slate-700 ml-2">
                    <?php if ($access_inventory): ?>
                        <a href="?page=laporan_gudang" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md hover:bg-slate-700 <?= $page == 'laporan_gudang' ? 'text-white' : 'text-slate-300' ?>">
                            <i class="fas fa-clipboard-list w-5 text-center mr-1"></i> Laporan Gudang
                        </a>
                        <a href="?page=laporan_aset" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md hover:bg-slate-700 <?= $page == 'laporan_aset' ? 'text-white' : 'text-slate-300' ?>">
                            <i class="fas fa-coins w-5 text-center mr-1"></i> Laporan Aset
                        </a>
                    <?php endif; ?>
                    <?php if ($access_finance): ?>
                        <a href="?page=laporan_keuangan" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md hover:bg-slate-700 <?= $page == 'laporan_keuangan' ? 'text-white' : 'text-slate-300' ?>">
                            <i class="fas fa-balance-scale w-5 text-center mr-1"></i> Laporan Pajak
                        </a>
                        <a href="?page=laporan_internal" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md hover:bg-slate-700 <?= $page == 'laporan_internal' ? 'text-white' : 'text-slate-300' ?>">
                            <i class="fas fa-file-invoice w-5 text-center mr-1"></i> Laporan Internal
                        </a>
                    <?php endif; ?>
                </div>
                ...
                <div id="menu-admin" class="<?= $is_menu_admin_open ? '' : 'hidden' ?> space-y-1 mt-1 pl-2 border-l-2 border-slate-700 ml-2">
                    <?php if ($access_inventory): ?>
                        <a href="?page=lokasi_gudang" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md hover:bg-slate-700 <?= $page == 'lokasi_gudang' ? 'text-white' : 'text-slate-300' ?>">
                            <i class="fas fa-map-marker-alt w-5 text-center mr-1"></i> Lokasi Gudang
                        </a>
                    <?php endif; ?>
                    
                    <?php if ($access_finance): ?>
                        <a href="?page=akun_kategori" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md hover:bg-slate-700 <?= $page == 'akun_kategori' ? 'text-white' : 'text-slate-300' ?>">
                            <i class="fas fa-tags w-5 text-center mr-1"></i> Akun & Kategori
                        </a>
                    <?php endif; ?>

                    <?php if ($access_user_mgmt): ?>
                        <a href="?page=manajemen_user" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md hover:bg-slate-700 <?= $page == 'manajemen_user' ? 'text-white' : 'text-slate-300' ?>">
                            <i class="fas fa-users w-5 text-center mr-1"></i> Manajemen User
                        </a>
                    <?php endif; ?>
                    <?php if ($access_settings): ?>
                        <a href="?page=system_logs" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md hover:bg-slate-700 <?= $page == 'system_logs' ? 'text-white' : 'text-slate-300' ?>">
                            <i class="fas fa-history w-5 text-center mr-1"></i> System Logs
                        </a>
                        <a href="?page=pengaturan" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md hover:bg-slate-700 <?= $page == 'pengaturan' ? 'text-white' : 'text-slate-300' ?>">
                            <i class="fas fa-cogs w-5 text-center mr-1"></i> Pengaturan
                        </a>
                        <a href="?page=update_system" class="group flex items-center px-2 py-2 text-sm font-medium rounded-md hover:bg-slate-700 <?= $page == 'update_system' ? 'text-white' : 'text-slate-300' ?>">
                            <i class="fas fa-sync-alt w-5 text-center mr-1"></i> Update Sistem
                        </a>
                    <?php endif; ?>
                </div>
            </nav>
    ...
    <main class="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-4 md:p-6">
            ...
            $filename = "views/" . $page . ".php";
            if (file_exists($filename)) {
                // Tampilkan Flash Message (Global)
